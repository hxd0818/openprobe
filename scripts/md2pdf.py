#!/usr/bin/env python3
"""
md2pdf.py — 将 Agent 生成的深度分析报告 (report.md) 转换为专业 PDF
...
def md_to_html(md_path):
    ...
    company = Path(md_path).parent.name
    ...
    version = ver_m.group(1).strip().lstrip("* :") if ver_m else "v9"
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .cover h1 { font-size: 28px; color: #1e3a5f; margin-bottom: 16px; font-weight: 800; letter-spacing: 2px; }
  .cover .subtitle { font-size: 15px; color: #475569; margin: 24px 0; }
  .cover .meta-box {
    display: inline-block; background: linear-gradient(135deg,#f0f7ff 0%%,#e8f0fe 100%%);
    border-radius: 12px; padding: 20px 40px; margin-top: 30px; text-align: left;
    font-size: 12.5px; color: #334155; line-height: 2;
  }
  .cover .meta-box strong { color: #1e40af; }
  ...
  #toc h2 { font-size: 16px; color: #1e3a5f; border: none; margin-bottom: 12px; }
  ...
  h1 { font-size: 22px; color: #1e3a5f; border-bottom: 3px solid #2563eb; padding-bottom: 10px; margin-top: 36px; font-weight: 700; page-break-after: avoid; }
  h2 { font-size: 16px; color: #1e40af; border-left: 4px solid #3b82f6; padding-left: 12px; margin-top: 28px; font-weight: 600; page-break-after: avoid; }
  h3 { font-size: 13.5px; color: #334155; margin-top: 20px; font-weight: 600; page-break-after: avoid; }
  h4 { font-size: 12.5px; color: #475569; margin-top: 16px; font-weight: 600; page-break-after: avoid; }
  table { width: 100%%; border-collapse: collapse; margin: 12px 0 14px 0; font-size: 11.5px; page-break-inside: avoid; }
  th, td { border: 1px solid #cbd5e1; padding: 7px 10px; text-align: left; vertical-align: top; }
  th { background: linear-gradient(180deg,#f1f5f9 0%%,#e2e8f0 100%%); font-weight: 600; color: #1e293b; font-size: 11px; }
  tr:nth-child(even) { background: #f8fafc; }
  tr:hover { background: #eff6ff; }
  p { margin: 6px 0; text-align: justify; }
  blockquote { background: #fefce8; border-left: 4px solid #eab308; padding: 10px 16px; margin: 12px 0; font-style: italic; color: #854d0e; border-radius: 0 8px 8px 0; page-break-inside: avoid; }
  hr { border: none; border-top: 2px solid #e2e8f0; margin: 20px 0; }
  strong, b { color: #0f172a; }
  em, i { color: #64748b; }
  code { background: #f1f5f9; padding: 1px 5px; border-radius: 3px; font-size: 11px; color: #dc2626; font-family: 'Consolas',monospace; }
  pre { background: #1e293b; color: #e2e8f0; padding: 14px 18px; border-radius: 8px; overflow-x: auto; font-size: 11px; page-break-inside: avoid; }
  ...
  .risk-med { color: #ca8a04; font-weight: 700; }
...
<div class="cover">
  <h1>🔍 OSINT 企业竞争格局<br>深度调查报告</h1>
  <div class="subtitle">目标：%s</div>
  <div class="meta-box">
    <strong>📋 报告版本:</strong> %s<br>
    <strong>📅 数据截止:</strong> %s<br>
    <strong>📊 调查方法:</strong> OSINT螺旋迭代法 (Agent驱动)<br>
    <strong>🔧 数据源:</strong> 百度AI搜索 + 百度百科 + WebFetch深挖<br>
    <strong>📄 生成时间:</strong> %s
  </div>
  <p class="footer-note">⚠️ 本报告基于公开信息(OSINT)生成，仅供研究参考，不构成投资建议</p>
</div>
""" % (company, company, version, date_str, datetime.now().strftime("%Y-%m-%d %H:%M"))
    ...
    html += '<p>OSINT Skill v9 | Agent驱动深度分析 | 数据源: 百度AI搜索 + 百度百科 + WebFetch</p>'
    html += '<p>⚠️ 本报告基于公开开源情报(OSINT)生成 · 仅供参考 · 不构成任何投资/商业决策建议</p>'
    html += '</div></body></html>'
